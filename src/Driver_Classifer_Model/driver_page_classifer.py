...
def driver_model():
    ...
    while not is_anomaly and ((counter + seq_length) < len(driver)):
        ...
        model = load_model_and_encoder(sample_driver.shape[1], 12, 60)
        criterion = nn.MSELoss()
        loss_pred = predict_loss(model, sample_driver, 60, criterion, threshold=0.03)
    ...
    if is_anomaly:
        st.error("Unauthorized driver detected!")
        st.write(f"Detected the anamolies after: {120 + counter} seconds" )
    else:
        st.success("Authorized driver detected!")
        st.write(f"No anamolies detected after: {120 + counter} seconds" )
